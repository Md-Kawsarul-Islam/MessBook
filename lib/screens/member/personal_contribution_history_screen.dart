...
import 'package:mess_manager/providers/expense_provider.dart'; // Manages contributions
...
class PersonalContributionHistoryScreen extends StatefulWidget {
  ...
  @override
  State<PersonalContributionHistoryScreen> createState() =>
      _PersonalContributionHistoryScreenState();
}

class _PersonalContributionHistoryScreenState
    extends State<PersonalContributionHistoryScreen> {
  ...
  Future<void> _fetchPersonalContributionHistory() async {
    ...
    try {
      ...
      if (memberId != null && hostelId != null) {
        _contributionHistory = await expenseProvider.fetchContributionsByMember(
          memberId,
          hostelId,
        );
      } else {
      ...
      debugPrint('Error fetching personal contribution history: $e');
      if (!mounted) return;
    ...
    return Scaffold(
      ...
      body:
          _isLoading
              ...
              ? const Center(
                child: Text('No contribution entries found for you.'),
              )
              : ListView.builder(
