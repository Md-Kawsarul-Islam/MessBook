...
import 'package:mess_manager/providers/expense_provider.dart'; // Handles contributions
...
class ManageContributionsScreen extends StatefulWidget {
  ...
  @override
  State<ManageContributionsScreen> createState() =>
      _ManageContributionsScreenState();
}
...
class _ManageContributionsScreenState extends State<ManageContributionsScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      await expenseProvider.fetchAllContributions(hostelId);
  ...
  Future<void> _saveContribution() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_editingContribution == null) {
        success = await expenseProvider.addContribution(contribution, hostelId);
      } else {
        success = await expenseProvider.updateContribution(
          contribution,
          hostelId,
        );
      }
  ...
  Future<void> _deleteContribution(String id) async {
    ...
    if (confirm == true) {
      ...
      final bool success = await expenseProvider.deleteContribution(
        id,
        hostelId,
      );
      if (success) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text(AppConstants.manageContributions)),
      body:
          _isLoading
                            ...
                            : ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return Card(
                                  ...
                                  child: ListTile(
                                    title: Text(
                                      '$memberName contributed ৳${contribution.amount.toStringAsFixed(2)}',
                                    ),
                                    subtitle: Column(
                                      crossAxisAlignment:
                                      ...
                                      children: [
                                        Text(
                                          'Date: ${DateHelpers.formatDate(contribution.contributionDate)}',
                                        ),
